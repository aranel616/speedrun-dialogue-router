...
describe("ChoiceItemNode", () => {
    ...
    it("takes the first line from an array and handles an empty first line", () => {
        const {rerender, container} = render(<ChoiceItemNode {...props({
            choiceName: "c",
            text: ["first line"],
            isTerminal: false
        })} />);
        expect(screen.getByText("first line")).toBeInTheDocument();
        rerender(<ChoiceItemNode {...props({
            choiceName: "c",
            text: [""],
            isTerminal: false
        })} />);
...
describe("LinearNode", () => {
    ...
    it("shows a placeholder when there is no text (and for an empty first line)", () => {
        const {rerender} = render(<LinearNode {...props({
            id: "L",
            isTerminal: false
        })} />);
        expect(screen.getByText("(no text)")).toBeInTheDocument();
        rerender(<LinearNode {...props({
            id: "L",
            text: [""],
            isTerminal: false
        })} />);
    ...
    it("truncates a long preview, takes the first array line, and marks terminal/selected/highlighted", () => {
        const {container, rerender} = render(<LinearNode {...props({
            id: "L",
            text: "y".repeat(60),
            isTerminal: true,
            highlighted: true
