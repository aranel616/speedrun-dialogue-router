...
describe("buildGraph", () => {
    it("returns the scriptId and a terminal linear node for a lone node", () => {
        ...
        expect(node(g, "A")).toMatchObject({type: "linear", text: "hi", isTerminal: true});
    });

    it("linear node with a string next → non-terminal + a linear edge", () => {
        const g = buildGraph("s", {A: {text: "x", next: "B"}, B: {text: "y"}});
    ...
    it("choice node emits a choice node + a choiceItem per branch, with edges", () => {
        ...
        expect(node(g, "A__ci0")).toMatchObject({type: "choiceItem", choiceName: "go", text: "hello", isTerminal: false});
    ...
    it("records a choice's set as a single object and as an array", () => {
        const script: Script = {
            A: {choices: [
                {name: "one", text: "a", set: {name: "f", type: "set", value: true}},
                {name: "two", text: "b", set: [{name: "g", type: "set", value: 1}, {name: "h", type: "set", value: "x"}]},
            ]},
        };
        const g = buildGraph("s", script);
        expect(node(g, "A__ci0")!.sets).toEqual([{name: "f", type: "set", value: true}]);
    ...
    it("single-variable eq fork → one conditionItem naming the flag, bare-value edge labels", () => {
        ...
        const g = buildGraph("s", script);
        expect(node(g, "A__cond")).toMatchObject({type: "conditionItem", condition: "flag"});
    ...
    it("single-variable non-eq fork keeps the operator in the label", () => {
        const script: Script = {
            A: {text: "x", next: [
                {name: "score", type: "gte", value: 10, node: "B"},
                {name: "score", type: "lt", value: 10, node: "C"},
            ]},
            B: {text: "b"}, C: {text: "c"},
        };
        const g = buildGraph("s", script);
    ...
    it("mixed-variable fork → one conditionItem per branch labelled 'name op value'", () => {
        const script: Script = {
            A: {text: "x", next: [
                {name: "x", type: "eq", value: 1, node: "B"},
                {name: "y", type: "gt", value: 2, node: "C"},
            ]},
            B: {text: "b"}, C: {text: "c"},
        };
        const g = buildGraph("s", script);
        expect(node(g, "A__cond0")).toMatchObject({type: "conditionItem", condition: "x = 1"});
        expect(node(g, "A__cond1")).toMatchObject({type: "conditionItem", condition: "y > 2"});
    ...
    it("falls back to the raw operator name in labels for an unknown op", () => {
        ...
        const single = buildGraph("s", {
            A: {text: "x", next: [{name: "x", type: bad, value: 1, node: "B"}, {name: "x", type: bad, value: 2, node: "C"}]},
            B: {text: "b"}, C: {text: "c"},
        });
        ...
        const mixed = buildGraph("s", {
            A: {text: "x", next: [{name: "x", type: bad, value: 1, node: "B"}, {name: "y", type: bad, value: 2, node: "C"}]},
            B: {text: "b"}, C: {text: "c"},
        });
    ...
    it("a conditional next on a choice branch also forks", () => {
        const script: Script = {
            A: {choices: [
                {name: "c", text: "t", next: [
                    {name: "f", type: "eq", value: true, node: "B"},
                    {name: "f", type: "eq", value: false, node: "C"},
                ]},
            ]},
            B: {text: "b"}, C: {text: "c"},
        };
