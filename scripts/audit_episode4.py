"""
Audit episode4.ts against the wiki wikitext source.
Prints the full mandatory dialogue flow (scene by scene) so we can spot
missing nodes and identify choice points.

Source: the Fandom "Episode 4: Dark Room - Script" page, saved as raw
...
WIKI = "/home/sarah/Episode 4_ Dark Room - Script _ Life is Strange Wiki _ Fandom.wikitext"
...
for raw_line in lines:
    ...
    # Catch optional-conversation markers even when the header line carries
    # trailing markup (e.g. '<u>Optional Conversation with David</u>'''<br />...).
    if re.search(r'optional conversation', line, re.I):
        skip_optional = True
        continue
