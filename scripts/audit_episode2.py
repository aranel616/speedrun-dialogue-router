"""
Audit episode2.ts against the wiki wikitext source.
Prints the full mandatory dialogue flow (scene by scene) so we can spot
missing nodes and identify choice points.

Source: the Fandom "Episode 2: Out of Time - Script" page, saved as raw
...
WIKI = "/home/sarah/Episode 2_ Out of Time - Script _ Life is Strange Wiki _ Fandom.wikitext"
...
for raw_line in lines:
    ...
    # Section heading -> ends any optional-skip region
    hm = HEADING.match(line)
    if hm:
        started = True
        skip_optional = False
        title = clean_inline(hm.group(2).replace("'''", ""))
        out.append("")
